...
# --- ÁREA LOGADA ---
    ...
    # --- PERFIL PROFESSOR ---
        ...
        if aulas_raw:
            ...
            # Filtro robusto pelo nome do professor
    ...
    # --- PERFIL DIREÇÃO ---
            ...
            if evs:
                for e in evs:
                    with st.expander(f"📝 {e.get('Nome do Evento')} ({formatar_data_pt(e.get('Data'))})"):
                        if st.button("Apagar Evento", key=f"del_ev_{e['_id']}"):
        ...
        with t3:
            ...
            if aulas: 
                df_all_aulas = pd.DataFrame(aulas)
                st.dataframe(df_all_aulas[['Professor', 'Aluno', 'DiaHora', 'Sala']], use_container_width=True)
    ...
    # --- PERFIL MAESTRO ---
        ...
        with t1:
            ...
            reps = base.list_rows("Repertorio")
            if reps:
                for r in reps:
                    col_txt, col_del = st.columns([4, 1])
                    col_txt.write(f"🎵 **{r.get('Nome da Obra')}** - {r.get('Compositor')}")
                    if col_del.button("🗑️", key=f"del_rep_{r['_id']}"):
        ...
        with t2:
            ...
            if evs:
                ...
                st.dataframe(df_evs[['Nome do Evento', 'Data', 'Hora']], use_container_width=True, hide_index=True)
            else: st.info("Sem eventos na agenda.")
